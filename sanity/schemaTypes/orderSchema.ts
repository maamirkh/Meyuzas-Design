...
export default defineType({
    ...
    defineField({
      ...
      options: {
        list: [
          ...
          {title: 'Online Payment (PayFast)', value: 'payfast'},
        ],
        layout: 'radio',
      },
      readOnly: true,
  ...
  preview: {
    ...
    prepare({title, subtitle, status, total, paymentStatus}) {
      const payTag = paymentStatus ? ` | 💳 ${paymentStatus.toUpperCase()}` : '';
      return {
        title: `${title} - Rs. ${total}`,
        subtitle: `[${status?.toUpperCase()}]${payTag} on ${new Date(subtitle).toLocaleString()}`,
      }
    },
  },
  orderings: [
